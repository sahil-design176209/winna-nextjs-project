...
interface BetSlipProps {
  selectedBet: { team: string; odd: string } | null;
  isCollapsed: boolean;
  ...
  return (
    <div className="hidden md:block w-full max-w-[320px] bg-black text-white font-sans rounded-t-lg overflow-hidden shadow-xl select-none transition-all duration-300">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-accent-blue">
        ...
        >
          <span className="font-bold text-lg tracking-wide text-white flex items-center justify-center gap-2">
            <ThemeIcons.BetsyIcon />
            <span> Betslip</span>
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation(); // Prevents double toggling since parent has onClick
